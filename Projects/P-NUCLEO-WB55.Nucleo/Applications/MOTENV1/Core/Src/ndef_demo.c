...
#define DEMO_RAW_MESSAGE_BUF_LEN      256 //[STM] - Limit buffer size to 256   /*!< Raw message buffer len     */
 ...
 ******************************************************************************
 */
uint32_t   jtTimer;                 //[STM]
rfalNfcDevice previous_nfcDevice;   //[STM]
extern uint8_t demoCycleEnable;     //[STM]
...
ndefConstBuffer  bufConstRawMessage;   //[STM] - Make global so it is accessible from app
...
bool demoIni( void )
{
...
#endif
    //ndefShowDemoUsage();  //[STM] - Disable different modes menu
    ...
    if( err == ERR_NONE )
    {
        ...
        discParam.techs2Find           = RFAL_NFC_POLL_TECH_A | RFAL_NFC_POLL_TECH_V;  //( RFAL_NFC_POLL_TECH_A | RFAL_NFC_POLL_TECH_B | RFAL_NFC_POLL_TECH_F | RFAL_NFC_POLL_TECH_V | RFAL_NFC_POLL_TECH_ST25TB );   //[STM] per Bruno

//[STM], per Bruno
//#if defined(ST25R3911) || defined(ST25R3916)
//        discParam.techs2Find   |= RFAL_NFC_POLL_TECH_AP2P;  //[STM], per Bruno
...
//    checkUserButton();
    //[STM] - Commented out to avoid conflicts with B1_Pin (PC4) 
//    /*******************************************************************************/
//    /* Check if USER button is pressed */
//    if( platformGpioIsLow(PLATFORM_USER_BUTTON_PORT, PLATFORM_USER_BUTTON_PIN))
//    {
                    ...
                    case RFAL_NFC_LISTEN_TYPE_NFCA:
                        ...
                        switch( nfcDevice->dev.nfca.type )
                        {
                            ...
                            case RFAL_NFCA_T4T:
                                ...
                                //[STM] - Per Bruno
                                if (platformTimerIsExpired(jtTimer))
                                {
                                  previous_nfcDevice.dev.nfca.nfcId1[0] = 0;
                                }                                
                                if(memcmp(previous_nfcDevice.dev.nfca.nfcId1, nfcDevice->dev.nfca.nfcId1, nfcDevice->nfcidLen) != 0)
                                {
                                  demoNdef(nfcDevice);                                  
                                }
                        ...
                        //previous_UID = nfcDevice->dev.nfca.nfcId1;
                        previous_nfcDevice = *nfcDevice;
                        jtTimer = platformTimerCreate(2000);
//                        while( rfalNfcaPollerCheckPresence(RFAL_14443A_SHORTFRAME_CMD_WUPA, &sensRes) == ERR_NONE )          //[STM] - Removed per Bruno
//                        {
...
//                            while (rfalNfcvPollerInventory( RFAL_NFCV_NUM_SLOTS_1, RFAL_NFCV_UID_LEN * 8U, nfcDevice->dev.nfcv.InvRes.UID, &invRes, &rcvdLen) == ERR_NONE)     //[STM] - Remove while loop. Not needed since tag will never by removed
//                            {
//                                platformDelay(130);
//                            }
                            demoCycleEnable = 0;   //[STM] - Disable the NFC Reader demoCycle
                        }
                        break;
                ...
                //rfalNfcDeactivate( false );   //[STM], Leave NFC reader activated so it's always energizing the smartag. per John T
...
static void demoNdef(rfalNfcDevice *pNfcDevice)
{
    ...
    //ndefConstBuffer  bufConstRawMessage;    //[STM] - made global so it is accesible from app
