...
static ReturnCode ndefMediaVCardTranslate(const ndefConstBuffer* bufText, ndefConstBuffer* bufTranslation)
{
    ...
    static const ndefTranslate translate[] =
    {
    ...
    {
        if (ST_BYTECMP(bufText->buffer, translate[i].vCardString, strlen((const char*)translate[i].vCardString)) == 0)
        {
            bufTranslation->buffer = translate[i].english;
            bufTranslation->length = strlen((const char*)translate[i].english);
...
ReturnCode ndefMediaVCardDump(const ndefType* vCard)
{
    ...
    static ndefConstBuffer bufTypePHOTO = { (uint8_t*)"PHOTO", sizeof("PHOTO") - 1U };

    static const ndefConstBuffer* bufVCardField[] =
    {
        &bufTypeN    ,
        &bufTypeFN   ,
        &bufTypeADR  ,
