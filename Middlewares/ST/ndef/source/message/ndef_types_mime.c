...
static const ndefConstBuffer bufDelimiterLineFeed  = { LINEFEED, sizeof(LINEFEED) - 1U};    /*!< "\n"   */
...
/*****************************************************************************/
static ReturnCode ndefVCardExtractLine(const ndefConstBuffer* bufPayload, ndefConstBuffer* bufLine, const ndefConstBuffer** bufDelim)
{
    ReturnCode err;
    ...
    *bufDelim = &bufDelimiterNewLine;
    
    err = ndefVCardFindMarker(bufPayload, &bufDelimiterNewLine, &markerOffset);
    if (err == ERR_NONE)
    {
    ...
    {
        err = ndefVCardFindMarker(bufPayload, &bufDelimiterLineFeed, &markerOffset);
        if (err == ERR_NONE)
        {
            *bufDelim = &bufDelimiterLineFeed; 
            /* Return up to the marker */
            bufLine->buffer = bufPayload->buffer;
            bufLine->length = markerOffset;
        }
...
static ReturnCode ndefVCardParseLine(const ndefConstBuffer* bufLine, ndefVCardEntry* vCardEntry)
{
    ...
    vCardEntry->bufValueLength = (uint16_t)(bufLine->length - colonOffset - 1U);
...
static ReturnCode ndefPayloadToVcard(const ndefConstBuffer* bufPayload, ndefType* vCard)
{
    ...
    const ndefConstBuffer *bufDelim;
    ...
    while ( (offset < bufPayload->length) && (entry_count < NDEF_VCARD_ENTRY_MAX) )
    {
        ...
        (void)ndefVCardExtractLine(&bufLinePayload, &bufLine, &bufDelim);
        ...
        offset += bufLine.length + bufDelim->length;
    }
